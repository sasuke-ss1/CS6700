...
def train(args,env, use_max = None):
    gamma = args.gamma
    ...
    QNet = DDQN(n_state, n_action, [hidden_size1, hidden_size2], use_max=use_max).to(device)
    targetQNet = DDQN(n_state, n_action, [hidden_size1, hidden_size2], use_max=use_max).to(device)
...
if __name__ == '__main__':
    ...
    if args.wandb == False:
        ...
        for i in range(num_expts):
            ...
            episodic_rewards_list_AVG.append(episodic_rewards)
            episodic_rewards = train(args,env,use_max = True)
            episodic_rewards_list_MAX.append(episodic_rewards)
